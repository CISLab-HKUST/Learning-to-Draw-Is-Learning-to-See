...
def init_displacement_field_TPS(image, uid):
  with open("Data/final_fiducials.json") as f:
    fiducials = json.load(f)
  target = image[:-4] + '_' + uid + '.png'
  src_db = fiducials[target]["dst"]
  dst_db = fiducials[target]["src"]
  
  #for ac
  #src_db, dst_db = dst_db, src_db
  
  src = []
  dst = []
  for xy_src, xy_dst in zip(src_db, dst_db):
    if xy_dst != "":
...
fixed_0_np = cv2.imread("Data/sketches_png/%s_%s.png" % (image[:-4], uid), cv2.IMREAD_GRAYSCALE)
...
cv2.imwrite("Data/register_input1/%s_%s_input1.png" % (image[:-4], uid), 255 - fixed_0_np)
fixed_0 = sitk.GetImageFromArray(fixed_0_np.astype(np.float32))

moving_0_np = 255 - cv2.imread("Data/anime_style/%s_out.png" % image[:-4], cv2.IMREAD_GRAYSCALE)
# moving_0_np = 255 - cv2.imread("./imgprop/imgprop_%s.png" % image[:-4], cv2.IMREAD_GRAYSCALE)
moving_0_np = cv2.resize(moving_0_np, (256, 256))
cv2.imwrite("Data/register_input2/%s_%s_input2.png" % (image[:-4], uid), 255 - moving_0_np)
moving_0 = sitk.GetImageFromArray(moving_0_np.astype(np.float32))

# for accuracy
fixed_0, moving_0 = moving_0, fixed_0
...
sitk.WriteTransform(outTx, "./Data/transform_labeled/transform_%s_%s.txt" % (image[:-4], uid))
...
if ( not "SITK_NOSHOW" in os.environ ):
    ...
    cv2.imwrite("./Data/visual_labeled/%s_%s_output.png" % (image[:-4], uid), 255 - out_0_np)
